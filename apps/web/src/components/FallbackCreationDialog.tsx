...
interface FallbackData {
  ...
  repoSources?: string[];
  /** For workspaces: projectId from the original request */
  projectId?: string;
  prompt: string;
}
...
export function FallbackCreationDialog() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (isProject) {
        ...
        // Create workspace with manual data
  ...
  return (
    <DialogShell
      open
      title={isProject ? "Create Project Manually" : "Create Workspace Manually"}
      maxWidth="max-w-lg"
      onClose={handleClose}
    >
      <div className="space-y-4">
        ...
        >
          {isProject ? "Create Project" : "Create Workspace"}
        </Button>
      </div>
    </DialogShell>
  );
