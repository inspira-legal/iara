...
export function useCreationToasts(): void {
  ...
  useEffect(() => {
        ...
        case "created": {
          if (toastId && name) {
            const updates: Parameters<typeof updateToast>[1] = {
              message: `${label === "project" ? "Project" : "Workspace"} ${name} created`,
            ...
            if (entityId) {
              updates.action = {
                ...
                onClick: () => {
                  if (type === "workspace") {
                    selectWorkspace(entityId);
                  } else {
                    selectWorkspace(`${entityId}/main`);
                  }
                },
              };
            }
    ...
    return unsub;
  }, [addListener, toastPersistent, updateToast, dismissToast, selectWorkspace]);
}
